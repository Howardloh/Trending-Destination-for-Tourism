...
# Multi-line string containing the country data
country_data = """
...
# Parsing the TSV data
country_list = []
lines = country_data.strip().split('\n')  # Split the string into lines
for line in lines:
    parts = line.split('\t')  # Split each line by tab to get the parts
    if len(parts) == 4:  # Ensure each line has exactly four parts
...
json_data = json.dumps(country_list, indent=4)

# Write the JSON data to a file
with open('country_codes.json', 'w') as file:
    file.write(json_data)

print("Countries data has been written to 'country_codes.json'")